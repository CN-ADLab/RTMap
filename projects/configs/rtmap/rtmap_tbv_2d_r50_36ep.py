...
point_cloud_range = [-36.0, -18.0, -5.0, 36.0, 18.0, 3.0]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(
            ...
            z_cfg=z_cfg,
            post_center_range=[-40, -25, -35, -25, 40, 25, 40, 25],
            pc_range=point_cloud_range,
            max_num=50,
            voxel_size=voxel_size,
            num_classes=num_map_classes),
        ...
        loss_pts=dict(type='PtsFusionLoss', loss_weight_nll=0.03, loss_weight_l1=5.0),
        ...
        loss_pv_seg=dict(type='SimpleLoss', 
                    pos_weight=1.0,
                    loss_weight=2.0),
        loss_pose=dict(type='PoseLoss', translation_weight=0.04, loss_weight=1.0),),
...
dataset_type = 'CustomTbVOfflineLocalMapDataset'
data_root = 'data/tbv/'
file_client_args = dict(backend='disk')


train_pipeline = [
    dict(type='CustomLoadMultiViewImageFromFiles', to_float32=True, padding=True, fit_tbv_to_av2=True),
    dict(type='RandomScaleImageMultiViewImage', scales=[0.3], fit_tbv_to_av2=True),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'tbv_map_infos_train.pkl',
        z_cfg=z_cfg,
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'tbv_map_infos_synthetic_val.pkl',
        map_ann_file=data_root + 'tbv_map_infos_synthetic_val.json',
        # code_size=3,
        z_cfg=z_cfg,
        load_interval=4,
...
lr_config = dict(
    ...
    min_lr_ratio=1e-3)
total_epochs = 36
evaluation = dict(interval=36, pipeline=test_pipeline, metric='chamfer')
# total_epochs = 50
