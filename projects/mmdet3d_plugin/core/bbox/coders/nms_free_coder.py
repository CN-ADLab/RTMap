...
@BBOX_CODERS.register_module()
class RTMapNMSFreeCoder(BaseBBoxCoder):
    """Bbox coder for NMS-free detector.
    Args:
        pc_range (list[float]): Range of point cloud.
        post_center_range (list[float]): Limit of the center.
    ...
    def decode_single(self, cls_scores, bbox_preds, pts_preds, pts_betas, pose_preds):
        """Decode bboxes.
        Args:
            cls_scores (Tensor): Outputs from the classification head, \  (50,3)
                shape [num_query, cls_out_channels]. Note \
                cls_out_channels should includes background.
            bbox_preds (Tensor): Outputs from the regression \
                head with normalized coordinate format (cx, cy, w, l, cz, h, rot_sine, rot_cosine, vx, vy). \
        ...
        cls_scores = cls_scores.sigmoid()  # (50,3)
        scores, indexs = cls_scores.view(-1).topk(max_num)  # 50, 50
        labels = indexs % self.num_classes
        bbox_index = indexs // self.num_classes
        bbox_preds = bbox_preds[bbox_index]
        pts_preds = pts_preds[bbox_index]
        pts_betas = pts_betas[bbox_index] if pts_betas != None else None
        ...
        final_pts_betas = denormalize_2d_betas(pts_betas, self.pc_range) if pts_betas != None else None
        # final_box_preds = bbox_preds 
        final_scores = scores 
        final_preds = labels 
        final_poses = pose_preds
        ...
        if self.post_center_range is not None:
            ...
            labels = final_preds[mask]
            betas = final_pts_betas[mask] if pts_betas != None else None
            poses = final_poses if pose_preds != None else None
    ...
    def decode(self, preds_dicts):
        ...
        all_beta_preds = preds_dicts['all_beta_preds'][-1]
        all_pose_preds = preds_dicts['all_pose_preds'][-1]
        batch_size = all_cls_scores.size()[0]
        predictions_list = []
        for i in range(batch_size):
            predictions_list.append(
                self.decode_single(
                    ...
                    all_bbox_preds[i],
                    all_pts_preds[i],
                    all_beta_preds[i] if all_beta_preds is not None else None,
                    all_pose_preds[i] if all_pose_preds is not None else None,
            ))
        return predictions_list
