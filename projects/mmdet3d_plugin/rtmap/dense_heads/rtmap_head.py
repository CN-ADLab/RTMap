...
class pose_branch(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256):
        ...
        self.fc_pose = nn.Linear(hidden_dim, 3)

    def forward(self, x, prior_embeds):
        new_embeds = self.query_project(prior_embeds)
        h = x + new_embeds
        h = self.encoder(h)
        h, _ = torch.max(h, dim=1, keepdim=True)
        h = self.fc_pose(h)
...
class new_reg_branch_laplace(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256, use_shared_mlp=False, code_size=2):
        super().__init__()
        if use_shared_mlp:
            ...
            self.layers_means = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, hidden_dim),
                nn.ReLU(),
            )
            self.layers_stds = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, hidden_dim),
                nn.ReLU(),
            )
        self.fc_means = nn.Linear(hidden_dim, code_size)
        self.fc_stds = nn.Linear(hidden_dim, code_size)
    ...
    def forward(self, x):
        if self.use_shared_mlp:
            ...
            h_means = self.layers_means(x)
            h_stds = self.layers_stds(x)
            means = self.fc_means(h_means)
            stds = torch.clamp(F.softplus(self.fc_stds(h_stds)), min=1e-5, max=10)
...
class RTMapHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.loss_pose = build_loss(loss_pose) if loss_pose is not None else None
    ...
    def _init_layers(self):
        ...
        reg_branch = new_reg_branch_laplace(code_size=self.code_size)
        ...
        self.query_project = nn.Sequential(
            nn.Linear(2 + self.num_classes, self.embed_dims * 2),
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None, only_bev=False, rtmap_prior=None):
        """Forward function.
        Args:
        ...
        # hierarchical query from MapTR
        if self.query_embed_type == 'all_pts':
            object_query_embeds = self.query_embedding.weight.to(dtype)
        elif self.query_embed_type == 'instance_pts':
            pts_embeds = self.pts_embedding.weight.unsqueeze(0)
            instance_embeds = self.instance_embedding.weight[0:num_vec].unsqueeze(1)
            object_query_embeds = (pts_embeds + instance_embeds).flatten(0, 1).to(dtype).expand(bs, -1, -1)

        # prior query embeds
        if rtmap_prior is not None:
            rtmap_prior = torch.stack(rtmap_prior)
        else:
            rtmap_prior = torch.zeros(self.num_vec * self.num_pts_per_vec, 5).cuda() 
        
        if self.training:
            pts_instance_prior_embeds = self.query_project(rtmap_prior).repeat(1, self.k_one2many+1, 1) 
        else:
            pts_instance_prior_embeds = self.query_project(rtmap_prior)

        # hybrid query embeds
        object_query_embeds = object_query_embeds + pts_instance_prior_embeds
        ...
        for lvl in range(hs.shape[0]):
            ...
            reg_out = self.reg_branches[lvl](hs[lvl])
            if not self.z_cfg['gt_z_flag']:
                tmp = reg_out[:, :, 0:2]
                beta = reg_out[:, :, 2:4]
                outputs_pts_beta = beta.view(beta.shape[0], num_vec, self.num_pts_per_gt_vec, 2)
            else:
                tmp = reg_out[:, :, 0:3]
                beta = reg_out[:, :, 3:4]
                outputs_pts_beta = beta.view(beta.shape[0], num_vec, self.num_pts_per_gt_vec, 3)

            tmp += reference
            tmp = tmp.sigmoid()

            outputs_coord, outputs_pts_coord = self.transform_box(tmp,num_vec=num_vec)
            outputs_pose = self.pose_branches[lvl](hs[lvl], pts_instance_prior_embeds).squeeze(1)
        ...
        outputs_pts_betas = torch.stack(outputs_pts_betas)
        ...
        outs = {
            ...
            "all_pose_preds": outputs_poses,
            "all_beta_preds": outputs_pts_betas,
        }
    ...
    def get_targets(self,
        ...
        prior_indices_list = [prior_indices_list for _ in range(num_imgs)] if prior_indices_list == None else prior_indices_list
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         ...
         pos_inds_list, neg_inds_list) = multi_apply(
            ...
            gt_labels_list, gt_bboxes_list, gt_shifts_pts_list, gt_bboxes_ignore_list, prior_indices_list)
    ...
    def loss_single(self,
        ...
        # import pdb;pdb.set_trace()
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,pts_preds_list, gt_bboxes_list, 
        ...
        ## 3. pts loss
        normalized_pts_targets = normalize_2d_pts(pts_targets, self.pc_range) if not self.z_cfg['gt_z_flag'] else normalize_3d_pts(pts_targets, self.pc_range)  # num_samples, num_order, num_pts, num_coords
        pts_preds = pts_preds.reshape(-1, pts_preds.size(-2),pts_preds.size(-1))  # num_samples, num_pts, num_coords
        ...
        pts_beta = pts_beta.reshape(-1, pts_beta.size(-2), pts_beta.size(-1))
        out = torch.cat([pts_preds, pts_beta], dim=2)  # (bs*25, 20, 2+2/2+3)

        isnotnan = torch.isfinite(normalized_pts_targets).all(dim=-1).all(dim=-1)
        ...
        ## 5. pose loss
        loss_pose = self.loss_pose(pred_pose, gt_pose)
        ...
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            ...
            loss_pose = torch.nan_to_num(loss_pose)
            if isinstance(loss_pts, dict):
                loss_pts['loss_nll'] = torch.nan_to_num(loss_pts['loss_nll'])
                loss_pts['loss_l1'] = torch.nan_to_num(loss_pts['loss_l1'])
            else:
                loss_pts = torch.nan_to_num(loss_pts)

        return loss_cls, loss_bbox, loss_iou, loss_pts, loss_dir, loss_pose
    ...
    def loss(self,
        ...
        all_pose_preds = preds_dicts['all_pose_preds']
        all_beta_preds   = preds_dicts['all_beta_preds']
        prior_indices_list = [np.array(indice.squeeze(0).cpu()) for indice in prior_indices_list] if prior_indices_list != None else None
        gt_poses_list = torch.stack(gt_poses_list).squeeze(1) if gt_poses_list != None else None
        ...
        all_prior_indices_list = [prior_indices_list for _ in range(num_dec_layers)]
        all_gt_poses_list = [gt_poses_list for _ in range(num_dec_layers)]


        # import pdb;pdb.set_trace()
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir, losses_pose = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,all_pts_preds, all_gt_bboxes_list, 
            all_gt_labels_list,all_gt_shifts_pts_list, all_gt_bboxes_ignore_list,
            all_prior_indices_list, all_pose_preds, all_gt_poses_list, all_beta_preds)
        ...
        if isinstance(losses_pts[0], dict):
            loss_dict['loss_pts_nll'] = losses_pts[-1]['loss_nll']
            loss_dict['loss_pts_l1'] = losses_pts[-1]['loss_l1']
        else:
            loss_dict['loss_pts'] = losses_pts[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_pts_i, loss_dir_i, loss_pose_i in zip(
                                           losses_cls[:-1],
                                           losses_bbox[:-1],
                                           losses_iou[:-1],
                                           losses_pts[:-1],
            ...
            if isinstance(loss_pts_i, dict):
                loss_dict[f'd{num_dec_layer}.loss_pts_nll'] = loss_pts_i['loss_nll']
                loss_dict[f'd{num_dec_layer}.loss_pts_l1'] = loss_pts_i['loss_l1']
            else:
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            ret_list.append([bboxes, scores, labels, pts, betas, poses])
