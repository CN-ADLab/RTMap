...
class PoseLoss(nn.Module):
    ...
    def __init__(self, beta=1.0, reduction='mean', loss_weight=1.0, translation_weight=0.04):
    ...
    def forward(self,
        ...
        loss_yaw = smooth_l1_loss(
            ...
            beta=self.beta,
            reduction=reduction)
        loss = self.loss_weight * (self.translation_weight * loss_xy + loss_yaw)
        return loss
...
def pts_nll_loss_laplace(pred, target):
    ...
    pred_reshape = pred.reshape(sample_size, -1)      # [x_mean, y_mean, [z_mean], beta_x, beta_y, [beta_z]]

    if target.shape[-1] == 2:
        pts_pred = pred_reshape[:, :2]
        pts_betas = pred_reshape[:, 2:4]
        target = target.reshape(sample_size, -1)
    else:
        pts_pred = pred_reshape[:, :3]
        pts_betas = pred_reshape[:, 3:6]
        target = target.reshape(sample_size, -1)
...
@LOSSES.register_module()
class PtsFusionLoss(nn.Module):
    def __init__(self, reduction='mean', loss_weight_nll=0.03, loss_weight_l1=5.0):
        super(PtsFusionLoss, self).__init__()
        self.reduction = reduction
    ...
    def forward(self,
        ...
        coor_dim = target.shape[-1]
        loss_l1  = self.loss_weight_l1 * pts_l1_loss(pred[... ,:coor_dim], target, weight, reduction=reduction, avg_factor=avg_factor)

        weight = weight.reshape(weight.shape[0]*weight.shape[1], -1)  # 200, 20, 2 -> 4000, 2
        loss_nll = self.loss_weight_nll * pts_nll_loss_laplace(pred, target, weight, reduction=reduction, avg_factor=avg_factor)

        loss_dict = {'loss_nll':loss_nll, 'loss_l1':loss_l1}
        return loss_dict