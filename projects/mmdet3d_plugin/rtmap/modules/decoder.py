    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                tmp = reg_branches[lid](output)  # (bs, pts_num, 2/3/4/6)

                # assert reference_points.shape[-1] == 2

                new_reference_points = torch.zeros_like(reference_points)  # 2/3
                if reference_points.shape[-1] == 2:
                    new_reference_points = tmp[..., :2] + inverse_sigmoid(reference_points)
                else:
                    new_reference_points = tmp[..., :3] + inverse_sigmoid(reference_points)
