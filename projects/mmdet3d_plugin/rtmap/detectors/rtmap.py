...
class RTMap(MVXTwoStageDetector):
    def __init__(self,
                 use_grid_mask=False,
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, img_metas, prev_bev, only_bev=False, rtmap_prior=rtmap_prior)
        ...
        losses_pts = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas, prior_indices_list=prior_indices_list, gt_poses_list=gt_delta_pose)
    ...
    def forward_train(self,
        ...
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            ...
                                            gt_delta_pose, rtmap_prior, prior_indices_list,)
    ...
    def pred2result(self, bboxes, scores, labels, pts, betas, poses, attrs=None):
        ...
        result_dict = dict(
            ...
            pts_3d=pts.to('cpu'),
            betas_3d=betas.to('cpu') if betas is not None else None,
            poses_3d=poses.to('cpu') if poses is not None else None,
            )
        ...
        return result_dict
    def simple_test_pts(self, x, lidar_feat, img_metas, prev_bev=None, rescale=False, rtmap_prior=None):
        """Test function"""
        outs = self.pts_bbox_head(x, lidar_feat, img_metas, prev_bev=prev_bev, rtmap_prior=rtmap_prior)

        bbox_list = self.pts_bbox_head.get_bboxes(
            outs, img_metas, rescale=rescale)
        
        bbox_results = [
            self.pred2result(bboxes, scores, labels, pts, betas, poses)
            for bboxes, scores, labels, pts, betas, poses in bbox_list
        ]
        # import pdb;pdb.set_trace()
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        rtmap_prior = kwargs['rtmap_prior'] if 'rtmap_prior' in kwargs else None

        bbox_list = [dict() for i in range(len(img_metas))]
        new_prev_bev, bbox_pts = self.simple_test_pts(
            img_feats, lidar_feat, img_metas, prev_bev, rescale, rtmap_prior)
