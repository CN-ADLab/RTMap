...
class RandomScaleImageMultiViewImage(object):
    ...
    def __init__(self, scales=[], fit_tbv_to_av2=False):
        self.scales = scales
        self.fit_tbv_to_av2 = fit_tbv_to_av2
    ...
    def __call__(self, results):
        ...
        if not self.fit_tbv_to_av2:
            ...
            y_size = [int(img.shape[0] * rand_scale * 2) if img.shape[0]==1024 else int(img.shape[0] * rand_scale) for img in results['img']]
            x_size = [int(img.shape[1] * rand_scale * 2) if img.shape[1]==1024 else int(img.shape[1] * rand_scale) for img in results['img']]
            scale_factor = np.eye(4)
            scale_factor[0, 0] *= rand_scale * 2
            scale_factor[1, 1] *= rand_scale * 2
            scale_factor_front_center = np.eye(4)
            scale_factor_front_center[0, 0] *= rand_scale
            scale_factor_front_center[1, 1] *= rand_scale

            results['img'] = [mmcv.imresize(img, (x_size[idx], y_size[idx]), return_scale=False) for idx, img in enumerate(results['img'])]

            lidar2img = []
            img_aug_matrix = []
            for idx, l2i in enumerate(results['lidar2img']):
                if idx == 0:
                    cur_lidar2img = scale_factor_front_center @ l2i
                    cur_img_aug_matrix = scale_factor_front_center
                else:
                    cur_lidar2img = scale_factor @ l2i
                    cur_img_aug_matrix = scale_factor
                lidar2img.append(cur_lidar2img)
                img_aug_matrix.append(cur_img_aug_matrix)
