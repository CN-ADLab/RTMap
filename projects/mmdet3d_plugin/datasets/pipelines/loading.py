...
class CustomLoadMultiViewImageFromFiles(object):
    ...
    def __init__(self, to_float32=False, padding=True,pad_val=128, color_type='unchanged', fit_tbv_to_av2=False):
    ...
    def __call__(self, results):
        ...
        size = (max_h, max_w)
        if self.fit_tbv_to_av2:
            img_list = [mmcv.impad(img, shape=(1024,1024), pad_val=self.pad_val) if img.shape[:2] != (2048,1550) else mmcv.impad(img, shape=(2048,2048), pad_val=self.pad_val) for img in img_list]
            if self.to_float32:
                img_list = [img.astype(np.float32) for img in img_list]
            results['img'] = img_list
            results['img_shape'] = (2048,2048,3,7)
            results['ori_shape'] = (2048,2048,3,7)
            results['pad_shape'] = (2048,2048,3,7)
            num_channels = 3
        else:
            ...
            if self.to_float32:
                img = img.astype(np.float32)  # av2 (2048,2048,3,7)
            # unravel to list, see `DefaultFormatBundle` in formating.py
            # which will transpose each image separately and then stack into array
            results['img'] = [img[..., i] for i in range(img.shape[-1])]  # [array(2048,2048,3), ...]
