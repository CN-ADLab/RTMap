...
class VectorizedTbVLocalMap(object):
    CLASS2LABEL = {
        'divider': 0,
        'ped_crossing': 1,
        'boundary': 2,
    ...
    def generate_rtmap_prior(self, anns_results=None):
        vecs_query_num = self.num_vecs
        pts_query_num = self.fixed_num
        etmap_num_classes = len(self.vec_classes)
        etmap_num_coords = self.code_size

        prior_init = torch.zeros(vecs_query_num, pts_query_num, etmap_num_coords+etmap_num_classes)
        prior_indices_list = [np.array(anns_results['prior_indices_list'])]

        if len(anns_results['noise_gt_vecs_label']) > 0:
            prior_noise = anns_results['noise_gt_vecs_pts_loc'].shift_fixed_num_sampled_points_v2[:,0,:,:]
            noise_vecs_label = torch.tensor(anns_results['noise_gt_vecs_label'])

            prior_labels_embs = F.one_hot(noise_vecs_label, num_classes=etmap_num_classes).unsqueeze(1).expand(-1,pts_query_num,-1)
            
            prior_noise_embs = torch.cat([prior_noise, prior_labels_embs], dim=2)[:50]
            rtmap_prior = torch.cat([prior_noise_embs, prior_init[prior_noise.shape[0]:]], dim=0).reshape(-1, etmap_num_coords+etmap_num_classes)
        
        else:
            rtmap_prior = prior_init.reshape(-1, etmap_num_coords+etmap_num_classes)
            prior_indices_list = []
        
        anns_results['rtmap_prior'] = rtmap_prior.squeeze()
        anns_results['prior_indices_list'] = prior_indices_list

        return anns_results


    def gen_vectorized_samples(self, map_annotation, noise_map_annotation, gt_delta_pose, e2g_t, e2g_r, example=None, feat_down_sample=32):
        '''
        use lidar2global to get gt map layers
        '''
        is_changed_flag = {'divider': noise_map_annotation['divider_is_changed'], 'ped_crossing': noise_map_annotation['ped_crossing_is_changed']}
        vectors = []
        vectors_origin = []
        for vec_class in self.vec_classes:
            ...
            for i, instance in enumerate(instance_list):
                if instance.shape[0] < 2:
                    continue
                if vec_class not in is_changed_flag or not is_changed_flag[vec_class][i]:
                    vectors.append((LineString(np.array(instance)), self.CLASS2LABEL.get(vec_class, -1)))
                vectors_origin.append((LineString(np.array(instance)), self.CLASS2LABEL.get(vec_class, -1)))
        ...
        # noise prior
        is_prior_flag = {'divider': noise_map_annotation['prior_idx_divider'], 
                         'ped_crossing': noise_map_annotation['prior_idx_ped_crossing'], 
                         'boundary': noise_map_annotation['prior_idx_boundary']}
        noise_vectors = []
        noise_vectors_origin = []
        for vec_class in self.vec_classes:
            instance_list = noise_map_annotation[vec_class]
            for i, instance in enumerate(instance_list):
                if instance.shape[0] < 2:
                    continue
                if is_prior_flag[vec_class][i]:
                    noise_vectors.append((LineString(np.array(instance)), self.CLASS2LABEL.get(vec_class, -1))) 
                noise_vectors_origin.append((LineString(np.array(instance)), self.CLASS2LABEL.get(vec_class, -1))) 
        noise_gt_labels = []
        noise_gt_instance = []
        for instance, instance_type in noise_vectors:
            if instance_type != -1:
                noise_gt_instance.append(instance)
                noise_gt_labels.append(instance_type)
        noise_gt_instance = LiDARInstanceLines(noise_gt_instance, noise_gt_labels, self.sample_dist,
                        self.num_samples, self.padding, self.fixed_num,self.padding_value, patch_size=self.patch_size, code_size=self.code_size)

        # prior_idx[i] indicates the index of the ground truth instance corresponding to prior[i]. A value of -1 means that the prior is a synthetic instance.
        def update_corr(original_a, original_b, a_new, b_new):
            original_corr = list(range(len(original_b)))
            a_orig_indices = []
            i = j = 0
            while i < len(original_a) and j < len(a_new):
                if original_a[i] == a_new[j]:
                    a_orig_indices.append(i)
                    j += 1
                i += 1
            
            b_orig_indices = []
            i = j = 0
            while i < len(original_b) and j < len(b_new):
                if original_b[i] == b_new[j]:
                    b_orig_indices.append(i)
                    j += 1
                i += 1
            
            a_index_map = {orig: new for new, orig in enumerate(a_orig_indices)}
            
            new_corr = []
            for orig_b_idx in b_orig_indices:
                orig_a_idx = original_corr[orig_b_idx]
                new_a_idx = a_index_map.get(orig_a_idx, -1)
                new_corr.append(new_a_idx)
            
            return new_corr
        
        prior_gt_idx_list = update_corr(vectors_origin, noise_vectors_origin, vectors, noise_vectors)
        # check z
        for idx_noise, idx_gt in enumerate(prior_gt_idx_list):
            if idx_gt != -1:
                noise = [pt[2] for pt in list(noise_vectors[idx_noise][0].coords)]
                gt = [pt[2] for pt in list(vectors[idx_gt][0].coords)]
                assert noise == gt

        # gt delta pose
        gt_lidar2global_rotation_matrix = e2g_r
        gt_lidar2global_translation = e2g_t

        noise_translation_x, noise_translation_y, theta = gt_delta_pose
        noise_rotation_matrix_3d = np.array([[np.cos(theta),  -np.sin(theta), 0],
                                             [np.sin(theta), np.cos(theta),   0],
                                             [0,             0,               1]], dtype=np.float32)
        noise_translation_matrix = np.array([noise_translation_x, noise_translation_y, 0], dtype=np.float32)

        init_lidar2global_rotation = gt_lidar2global_rotation_matrix @ noise_rotation_matrix_3d
        init_lidar2global_translation = gt_lidar2global_translation + gt_lidar2global_rotation_matrix @ noise_translation_matrix
        ...
        anns_results = dict(
            ...
            noise_gt_vecs_pts_loc=noise_gt_instance,
            noise_gt_vecs_label=noise_gt_labels,

            gt_delta_pose=gt_delta_pose,
            init_pose_t=init_lidar2global_translation,
            init_pose_R=init_lidar2global_rotation,
            gt_pose_R=gt_lidar2global_rotation_matrix,
            gt_pose_t=gt_lidar2global_translation,
            prior_indices_list=prior_gt_idx_list,
        )
        return anns_results  
...
class CustomTbVOfflineLocalMapDataset(CustomNuScenesDataset):
    r"""NuScenes Dataset.
    ...
    def __init__(self,
        ...
        self.aux_seg = aux_seg
        self.vector_map = VectorizedTbVLocalMap(canvas_size=bev_size,
    ...
    def get_map_classes(cls, map_classes=None):
        ...
        return class_names
    def vectormap_pipeline(self, example, input_dict, is_train=True):
        '''
        ...
        anns_results = self.vector_map.gen_vectorized_samples(input_dict['ann_info'],
                    input_dict['noise_ann_info'], input_dict['gt_delta_pose'], 
                    input_dict['ego2global_translation'], input_dict['ego2global_rotation'],
                    example=example, feat_down_sample=self.aux_seg['feat_down_sample'])

        anns_results = self.vector_map.generate_rtmap_prior(anns_results=anns_results)
        ...
        # 1. gt_delta_pose
        gt_delta_pose = to_tensor(anns_results['gt_delta_pose'])
        example['gt_delta_pose'] = DC(gt_delta_pose, cpu_only=False)

        # 2. gt_pose & init_pose
        if is_train == False:
            gt_pose_R = to_tensor(anns_results['gt_pose_R'])
            gt_pose_t = to_tensor(anns_results['gt_pose_t'])
            init_pose_R = to_tensor(anns_results['init_pose_R'])
            init_pose_t = to_tensor(anns_results['init_pose_t'])

            example['gt_pose_R'] = DC(gt_pose_R, cpu_only=False)
            example['gt_pose_t'] = DC(gt_pose_t, cpu_only=False)
            example['init_pose_R'] = DC(init_pose_R, cpu_only=False)
            example['init_pose_t'] = DC(init_pose_t, cpu_only=False)

        # 3. rtmap_prior & prior_indices_list
        rtmap_prior = anns_results['rtmap_prior']
        prior_indices_list = to_tensor(anns_results['prior_indices_list'])

        example['rtmap_prior'] = DC(rtmap_prior, cpu_only=False)
        example['prior_indices_list'] = DC(prior_indices_list, cpu_only=False)
    ...
    def get_data_info(self, index):
        ...
        input_dict['noise_ann_info'] = info['noise_prior']
        input_dict['gt_delta_pose'] = info['gt_delta_pose']

        # shuffle data
        for c in ['divider', 'ped_crossing']:
            changed_flag_list_name = c + '_is_changed'
            prior_idx_list_name = 'prior_idx_' + c

            noise_ins_list = input_dict['noise_ann_info'][c]
            gt_ins_list = input_dict['ann_info'][c]
            changed_flag_list = input_dict['noise_ann_info'][changed_flag_list_name]
            prior_idx_list = input_dict['noise_ann_info'][prior_idx_list_name]

            new_indices = list(range(len(noise_ins_list)))
            random.shuffle(new_indices)
            input_dict['noise_ann_info'][c] = [noise_ins_list[i] for i in new_indices]
            input_dict['noise_ann_info'][changed_flag_list_name] = [changed_flag_list[i] for i in new_indices]
            input_dict['noise_ann_info'][prior_idx_list_name] = [prior_idx_list[i] for i in new_indices]
            input_dict['ann_info'][c] = [gt_ins_list[i] for i in new_indices]
    ...
    def prepare_test_data(self, index):
        ...
        self.pre_pipeline(input_dict)
        example = self.pipeline(input_dict)
        example = self.vectormap_pipeline(example, input_dict, is_train=False)
    ...
    def _format_gt(self):
        ...
        if (not os.path.exists(self.map_ann_file)) :
            ...
            for sample_id in range(dataset_length):
                ...
                gt_sample_dict = self.vectormap_pipeline(gt_sample_dict, input_dict, is_train=False)
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        res_path = osp.join(jsonfile_prefix, 'tbvmap_results.json')
    ...
    def _evaluate_single(self,
        ...
        for metric in metrics:
            ...
            for i, name in enumerate(self.MAPCLASSES):
                ...
                detail['TbVMap_{}/{}_AP'.format(metric,name)] =  cls_aps.mean(0)[i]
            print('map: {}'.format(cls_aps.mean(0).mean()))
            detail['TbVMap_{}/mAP'.format(metric)] = cls_aps.mean(0).mean()

            for i, name in enumerate(self.MAPCLASSES):
                for j, thr in enumerate(thresholds):
                    if metric == 'chamfer':
                        detail['TbVMap_{}/{}_AP_thr_{}'.format(metric,name,thr)]=cls_aps[j][i]
                    elif metric == 'iou':
